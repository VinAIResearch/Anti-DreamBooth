    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                pipeline = DiffusionPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    unet=accelerator.unwrap_model(unet).to(dtype=weight_dtype),
                    text_encoder=None if args.pre_compute_text_embeddings else accelerator.unwrap_model(text_encoder).to(dtype=weight_dtype),
                    revision=args.revision,
                ...
                if args.pre_compute_text_embeddings:
                    pipeline_args = {
                        "prompt_embeds": validation_prompt_encoder_hidden_states.to(dtype=weight_dtype),
                        "negative_prompt_embeds": validation_prompt_negative_prompt_embeds.to(dtype=weight_dtype),
                    }
                else:
